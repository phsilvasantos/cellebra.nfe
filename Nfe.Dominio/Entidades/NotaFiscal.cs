...

{
    public class NotaFiscal : Entidade
    {
        ...
        public string tPag { get; set; }// Diversos
        ...
        public int? idDesc { get; set; }//Identificador de local de destino da operação
        ...
        public int? indFinal { get; set; } //Indica operação com Consumidor final
        public int? indPres { get; set; } // Indicador de presença do comprador no estabelecimento comercial no momento da operação
...

        {
            ...
            //Não existe mais na versão 4.0 do layout da NFE
            ...
            XmlElement xdEmi = doc.CreateElement("dhEmi");
            xdEmi.InnerText = dEmi.ToString().Trim();
            XmlElement xtpNF = doc.CreateElement("tpNF");
            xtpNF.InnerText = tpNF.ToString().Trim();

            XmlElement xidDest = doc.CreateElement("idDest");
            Int16 idDest;
            if (Cliente.Endereco.CodPais.ToString() != "1058") //1058 = Brasil
                idDest = 3; //Exterior
            else if (Cliente.Endereco.UF != "SP")
                idDest = 2; //InterEstadual
            else 
                idDest = 1; //Estadual
            xidDest.InnerText = idDest.ToString().Trim();
            ...
            XmlElement xindFinal = doc.CreateElement("indFinal");
            xindFinal.InnerText = indFinal.ToString().Trim();
            XmlElement xindPres = doc.CreateElement("indPres");
            xindPres.InnerText = indPres.ToString().Trim();
            ...
            ide.AppendChild(xindFinal);
            ide.AppendChild(xindPres);
