...
namespace Nfe.Infra
{
    public class GerarCartaCorrecao
    {
        ...
        public bool PodeSerGerado(string Justificativa,int operacao)
        {
            ...
            if (Justificativa.Length < 1 || Justificativa.Length > 255) { err += 1; resp += "Preencha o motivo da carta de correção no campo justificativa!"; }
            else { this.xCorrecao = Justificativa; }
            if (err > 0)
            {
                resposta += resp;
                return false;                
        ...
        public void SalvaXML()
        {
            ...
            XmlElement dhEvento = doc.CreateElement("dhEvento"); dhEvento.InnerText = this.dhEvento.ToString("yyyy-MM-ddTHH:mm:ss") + this.dhEvento.ToString("%K");
            ...
            //xinfCanc.AppendChild(tpAmb);
            //xinfCanc.AppendChild(xServ);
            //xinfCanc.AppendChild(chNFE);
            //xinfCanc.AppendChild(nProt);
            //xinfCanc.AppendChild(xJust);
            //cancNFe.AppendChild(xinfCanc);
