    ...
    public class Servicos
    {
        ...
        public string gerarConsultaNota(string chave, int idNota, string justificativa = "")
        {
            try
            {
                ...
                StatusNFe status2 = new StatusNFe(chave, "2.00", "CONSULTAR");
                RepositorioNotaFiscal nota = new RepositorioNotaFiscal();
                //var novaNota = nota.GetById(idNota);
                
                //ServicoXML servico = new ServicoXML(novaNota);

                //config = new ConfiguracaoApp();
                //servico.config = config;
                //servico.PathRetornoLote = config.PastaXmlRetorno + "\\" + novaNota.IDChave + "-num-lot.xml";
                //servico.PathRetornoSituacaoErr = config.PastaXmlRetorno + "\\" + novaNota.IDChave + "-sit.err";

                //servico.GetLoteDaNota();
                //servico.getRetornoDoLote();
                ...
                else if(status.resposta.Equals("Cancelamento de NF-e homologado"))
                {
                    nota.AtualizarProtocolo(idNota, status.protocolo);
        ...
        public  string gerarConsultaStatusServico()
        {
            try
            {
                StatusServico novo = new StatusServico("2.00", "STATUS");
                novo.SalvaXML();
                novo.GetResposta();
        ...
        public  string GerarCancelamentoNota(int id, string Justificativa, string chave, int operacao)
        {
            try
            {
                GerarCancelamento cancel = new GerarCancelamento(id, chave);
