...
namespace Nfe.Infra
{
    public class GerarCancelamento
    {
        ...
        public GerarCancelamento(int idNota, string chave, string codigoEstado, string cnpj)
        {
            ...
            //this.tzd = "-03:00"; // UCT - Universal Coordinated Time (Normal)
            this.tzd = "-02:00"; // UCT - Universal Coordinated Time (No horário de verão)
        ...
        public bool PodeSerCancelado(string Justificativa,int operacao)
        {
            ...
            else if (ContasReceberExcluido(nota.idfechamento) == false) { err += 1; resp += "Solicite o cancelamento do contas a receber ao departamento Financeiro"; }
        ...
        public int NumeroSequenciaEvento(int id_NotaFiscal)
        {
            ...
            instrucao = " select count(id_notaFiscal) + 1 as numeroSequenciaEvento " +
                        ...
                        " and xmotivo not like '%Rejei[cç][aã]o%' ";
        ...
        private bool ContasReceberExcluido(int IDFechamento) {
            string instrucao;
            bool retorno = true;
            ...
            if (IDFechamento != 0)
            {
                ...
                instrucao = " select isnull(count(id_fechamento),0) as qtdcontasreceber " +
                            " from ContasReceber " +
                            " where id_fechamento = " + IDFechamento;

                SqlDataAdapter daContasReceber = new SqlDataAdapter(instrucao, conn);
                DataTable dtContasReceber = new DataTable();

                daContasReceber.Fill(dtContasReceber);
                DataRow drContasReceber = dtContasReceber.Rows[0];
                if (Convert.ToInt32(drContasReceber["qtdcontasreceber"].ToString()) > 0)
                    retorno = false;
                else
                    retorno = true;
        ...
        public bool verResposta(int idNota,string justificativa = "",string chave="")
        {
            bool retorno = false;
                    ...
                    foreach (XmlNode infRecNode in infCanc)
                     {
                        ...
                        RepositorioNotaFiscal rep = new RepositorioNotaFiscal();
                        StatusNFe status = new StatusNFe(chave, "2.00", "CONSULTAR");
                        ...
                        if (Convert.ToInt32(consStatServNodeElement.GetElementsByTagName("cStat")[0].InnerText) == (135) ||
                            Convert.ToInt32(consStatServNodeElement.GetElementsByTagName("cStat")[0].InnerText) == (155))
                         {
                             rep.AtualizarProtocolo(idNota, status.protocolo);
                             rep.AtualizarConfirmaCancelamento(idNota, justificativa, status.DataCancelamentoOuEnvioNFE);
                             rep.AtualizarStatus(idNota, status.resposta.ToString());
                         }
